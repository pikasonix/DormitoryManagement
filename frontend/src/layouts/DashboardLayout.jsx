...
const DashboardLayout = () => {
  ...
  const navigation = useMemo(() => {
    const allNavItems = [
      ...
      { name: 'Quản lý Đơn giá', href: '/fees', icon: Bars3Icon, roles: ['ADMIN', 'STAFF'] },
  ...
  const handleLogout = () => {
    ...
    logout();
  };  // --- Simplified getAvatarUrl function to always use default avatar ---
  const getAvatarUrl = () => {
            ...
            <button type="button" className="-m-2.5 p-2.5 text-gray-700 lg:hidden" onClick={() => setIsSidebarOpen(true)}>
              ...
              <div className="flex items-center gap-x-4 lg:gap-x-6 justify-end">
                <div className="relative">
                  ...
                  >
                    <span className="sr-only">Mở menu người dùng</span>                    {/* Hiển thị avatar - sửa lại xử lý URL đầy đủ */}
                    {(() => {
                      const avatarUrl = getAvatarUrl();

                      // Luôn hiển thị ảnh, nếu load lỗi thì fallback về defaultAvatar
                      return (
                        <img
