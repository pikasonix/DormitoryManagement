...
const PaymentForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                studentId: foundStudent.studentId,  // Sẽ được chuyển đổi thành studentProfileId ở backend
        ...
        <div className="space-y-6 max-w-lg mx-auto">
            ...
            <div>
                ...
                <h1 className="text-2xl font-semibold">
                    {isEditMode ? 'Chỉnh sửa Giao dịch Thanh toán' : 'Ghi nhận Thanh toán Thủ công'}
                </h1>
                {isEditMode && <p className="text-gray-500 mt-1">ID: {id}</p>}
            </div>
            <form onSubmit={handleSubmit} className="bg-white shadow sm:rounded-lg p-6 space-y-6">
                <div>
                    <Input label="Mã số Sinh viên *" id="studentId" name="studentId" value={formData.studentId} onChange={handleChange} required disabled={isLoading || isSaving} error={errors.studentId} autoComplete="off" />
                ...
                {foundStudent && (
                    <div className="space-y-2">
                        <div className="flex items-end gap-2">
                            ...
                            disabled={isLoading || isSaving || pendingInvoices.length === 0}
                        />
                ...
                <Textarea label="Ghi chú (Tùy chọn)" id="details" name="details" rows={3} value={formData.details} onChange={handleChange} disabled={isLoading || isSaving} error={errors.details} />
                <div className="flex justify-end gap-3 pt-5 border-t border-gray-200">
                    <Button variant="secondary" onClick={() => navigate('/payments')} disabled={isLoading || isSaving}>
                        Hủy
                    </Button>
                    <Button type="submit" isLoading={isSaving} disabled={isLoading || isSaving || !foundStudent}>
